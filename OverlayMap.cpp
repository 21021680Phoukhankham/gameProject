...
OverlayMap::OverlayMap(SDL_Renderer* renderer) {
    mRenderer = renderer;
...
bool OverlayMap::loadTileSheet(std::string path) {
    // Tải texture
    if (!mTileSheet->loadFromFile(path)) {
        std::cout << "Không thể tải tileset cho overlay: " << path << std::endl;
        return false;
    }
    ...
    std::cout << "Đã tải " << tileIndex << " tiles từ spritesheet cho overlay." << std::endl;
    return true;
}

bool OverlayMap::loadMap(std::string path) {
    std::ifstream mapFile(path);
    if (!mapFile.is_open()) {
        std::cout << "Không thể mở file overlay map: " << path << std::endl;
        return false;
    }
    ...
    std::cout << "Đã tải overlay map kích thước " << mMapWidth << "x" << mMapHeight << std::endl;
    return true;
}

void OverlayMap::render(SDL_Rect* camera) {
    ...
    if (camera == nullptr) {
        for (int y = 0; y < mMapHeight; y++) {
            for (int x = 0; x < mMapWidth; x++) {
                ...
                // Chỉ render những tile khác 0 (0 là tile trống)
        ...
        for (int y = startY; y < endY; y++) {
            for (int x = startX; x < endX; x++) {
                if (y < mMapHeight && x < mMapWidth) {
                    int tileType = mMap[y][x];
                    
                    // Chỉ render tile khác 0
                    if (tileType > 0 && tileType < (int)mTileClips.size()) {
                        // Xác định vị trí chính xác để vẽ tile
...
int OverlayMap::getTileWidth() {
    return mTileWidth;
}

int OverlayMap::getTileHeight() {
    return mTileHeight;
}

int OverlayMap::getMapWidth() {
    return mMapWidth;
}

int OverlayMap::getMapHeight() {
    return mMapHeight;
}